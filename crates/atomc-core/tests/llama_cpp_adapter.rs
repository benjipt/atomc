use atomc_core::llm::{LlamaCppClient, LlmError, LlmOptions, Prompt};
...
#[tokio::test]
async fn llama_cpp_client_parses_commit_plan() {
    let plan = json!({
        "schema_version": "v1",
        "plan": [
            {
...
#[tokio::test]
async fn llama_cpp_client_reports_non_success_status() {
...
#[tokio::test]
async fn llama_cpp_client_rejects_invalid_json() {
    let response = json!({ "choices": [{ "message": { "content": "not-json" } }] });
    let captured = Arc::new(Mutex::new(None));
...
#[tokio::test]
async fn llama_cpp_client_times_out() {
